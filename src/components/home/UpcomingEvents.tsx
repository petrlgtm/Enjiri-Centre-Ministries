  ...
  return (
    <div className="flex-1 rounded-xl bg-navy/70 px-1.5 py-2 sm:px-2 sm:py-2.5 text-center backdrop-blur-md border border-white/5">
      <motion.span
        ...
        className="block font-(family-name:--font-playfair) text-base sm:text-lg md:text-xl font-bold text-gold"
      >
                ...
                <div className="flex flex-col justify-center p-5 sm:p-8 md:w-[45%] md:p-10">
                  ...
                  <div className="mt-5 space-y-3">
                    {[
                      ...
                      <div key={`${item.text}-${i}`} className="flex items-center gap-2.5 text-sm text-(--gray-500)">
                        <item.icon className="shrink-0 text-gold/70" size={15} />
                        <span>{item.text}</span>
                      </div>
                    ))}
                  </div>
                  <p className="mt-5 text-[0.9rem] leading-relaxed text-(--gray-400)">
                    {event.description}
                  </p>
                </div>
              </div>
              <div className="relative h-0.5 w-full bg-(--gray-50)">
                <div className="absolute inset-y-0 left-1/2 w-0 -translate-x-1/2 bg-linear-to-r from-gold-light via-gold to-gold-light transition-all duration-700 group-hover:left-0 group-hover:w-full group-hover:translate-x-0" />
              </div>
            </motion.div>
          ))}
            ...
            {displayEvents.filter(e => !e.featured).map((event, index) => (
              ...
              >
                ...
                <div className="relative flex h-full flex-col justify-between p-6">
                  ...
                  {/* Bottom — text content */}
                  <div>
                    <h3 className="text-xl font-bold text-foreground transition-colors duration-300 group-hover:text-gold font-(family-name:--font-playfair)">
                      {event.title}
                    </h3>
